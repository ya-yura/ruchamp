...
async def create_next_round(
    ...
    while True:
        ...
        if len(winners) == 1:
            ...
            # Записываем победителя в таблицу WinnerTable
            await db.execute(
                insert(WinnerTable).values(
                    match_id=match_id,
            ...
            if len(third_place_participants) >= 2:
                third_place_fight = await create_fight(
                    ...
                    next_round, mat_vol, 1, is_third_place=True
                )
                await db.commit()
            else:
...
async def create_fight(db, match_id, pair, round_number, mat_vol, mat_counter, is_final=False, is_third_place=False):
    ...
    # Определение медалей и запись в таблицу WinnerTable
    if is_final:  # Финал
        await db.execute(
            insert(WinnerTable).values(
                match_id=match_id,
        ...
        await db.execute(
            insert(WinnerTable).values(
                match_id=match_id,
                winner_id=random_loser,
                winner_score='0',
                medal=2  # Серебро
            )
        )
    elif is_third_place:  # Бой за третье место
        await db.execute(
            insert(WinnerTable).values(
                match_id=match_id,
                winner_id=random_winner,
...
async def get_round_info(db, match_id, round_number):
    ...
    if round_number > 1:
        ...
        # Если в предыдущем раунде был только один бой (финал),
        # то текущий раунд - бой за третье место
        if previous_round_fights == 1:
            return {"name": "Бой за третье место", "fights": fight_infos}
